...
- matched=false ТОЛЬКО если на фото ЯВНО не крест этого блока: посторонний предмет/помещение/пейзаж/селфи/скриншот, пустой лист, случайное фото, или совершенно другой блок. Тогда мягко объясни, что нужно прислать именно рукописный крест блока.
НЕ придирайся к почерку, аккуратности, точности ссылок и порядку.
...
- Если на фото ЯВНО не то (пусто, посторонний предмет/помещение/пейзаж/скриншот, совсем другой блок) — matched=false, мягко подскажи прислать именно рукописный крест блока.
Верни СТРОГО JSON: {"matched": boolean, "feedback": string}
