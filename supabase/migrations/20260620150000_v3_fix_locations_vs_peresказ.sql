-- ИСПРАВЛЕНИЕ модели сдачи:
--  • Местописания (экран /locations → student_location_attempts) = РАЗОВОЕ
--    условие блока: все обязательные стихи закрыты видеокружком (medium=video_note passed).
--  • Пересказ (экран /recitation → student_block_recitations medium='audio') = ЕЖЕДНЕВНО.
-- «Закрытый день» = Фото + Молитва + Пересказ (3 задания за дату).
-- Блок сдан = >=7 закрытых дней + местописания завершены.
-- Ранее гейт проверял student_block_recitations video_note (никто не пишет) → баг.

CREATE OR REPLACE FUNCTION locations_complete(p_user_id UUID, p_block_id INTEGER)
RETURNS BOOLEAN LANGUAGE sql STABLE SECURITY DEFINER SET search_path = ''
AS $$
  SELECT NOT EXISTS (
    SELECT 1 FROM public.block_locations_to_recite l
    WHERE l.block_id = p_block_id AND l.is_required
      AND NOT EXISTS (
        SELECT 1 FROM public.student_location_attempts a
        WHERE a.user_id = p_user_id AND a.location_id = l.id
          AND a.medium = 'video_note' AND a.passed
      )
  );
$$;
REVOKE ALL ON FUNCTION locations_complete(UUID, INTEGER) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION locations_complete(UUID, INTEGER) TO service_role;
...
BEGIN
  ...
  RETURN (COALESCE(v_days, 0) >= 7 AND public.locations_complete(p_user_id, v_prev_id));
END; $$;
...
AS $$
  SELECT closed.block_id, count(*) AS days FROM (
    ...
    ) tasks GROUP BY block_id, d HAVING count(DISTINCT src) = 3
...
AS $$
  SELECT DISTINCT closed.user_id, closed.d FROM (
    ...
    ) tasks GROUP BY user_id, block_id, d HAVING count(DISTINCT src) = 3
  ) closed;
$$;
...
AS $$
  ...
  WHERE cd.days >= 7 AND public.locations_complete(cd.user_id, cd.block_id)
  GROUP BY cd.user_id;
$$;
