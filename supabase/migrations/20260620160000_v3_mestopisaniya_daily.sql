-- Местописания — ЕЖЕДНЕВНОЕ задание (нужно для закрытия каждого дня).
-- «Закрытый день» = Фото + Молитва + Пересказ + Местописания за одну дату.
-- Местописания «сделаны за дату D» = ВСЕ обязательные стихи блока закрыты
-- видеокружком (medium=video_note passed) в дату D.
-- (effective_date — для ускоренного тест-режима, виртуальные даты.)

ALTER TABLE public.student_location_attempts
...
BEGIN
  ...
  SELECT count(*) INTO v_req FROM public.block_locations_to_recite WHERE block_id = v_prev_id AND is_required;
  ...
  SELECT count(*) INTO v_days FROM (
    ...
    INTERSECT
    SELECT loc.d FROM (
      SELECT COALESCE(a.effective_date,(a.created_at AT TIME ZONE 'UTC')::date) AS d,
             count(DISTINCT a.location_id) AS done
      FROM public.student_location_attempts a
      JOIN public.block_locations_to_recite l ON l.id = a.location_id AND l.is_required AND l.block_id = v_prev_id
    ...
    WHERE loc.done >= v_req AND v_req > 0
  ) x;
    ...
    SELECT block_id, d FROM (
      ...
      UNION ALL
        SELECT loc.block_id, loc.d, 'loc' FROM (
          ...
          FROM public.student_location_attempts a
          JOIN public.block_locations_to_recite l ON l.id = a.location_id AND l.is_required
          WHERE a.user_id = p_user_id AND a.medium='video_note' AND a.passed
          GROUP BY l.block_id, COALESCE(a.effective_date,(a.created_at AT TIME ZONE 'UTC')::date)
        ) loc
        JOIN (SELECT block_id, count(*) req FROM public.block_locations_to_recite WHERE is_required GROUP BY block_id) rc
