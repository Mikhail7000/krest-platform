-- КРИТИЧНО: местописания-гейт требовал ВСЕ обязательные стихи видеокружком, но часть
-- стихов (practice_mode='daily_understanding'/'single_understanding') сдаётся ТОЛЬКО
-- аудио (видеокружок для них запрещён в роуте). done никогда не достигал req →
-- день не закрывался (Блок 1: 3/4, Блок 5: 7/8), closed_dates_all=0 у всех.
-- Фикс: видео-местописания требуют только стихи с practice_mode IS NULL.
...
CREATE OR REPLACE FUNCTION public.locations_complete(p_user_id uuid, p_block_id integer)
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path TO ''
AS $function$
  SELECT NOT EXISTS (
    SELECT 1 FROM public.block_locations_to_recite l
    WHERE l.block_id = p_block_id AND l.is_required AND l.practice_mode IS NULL
      AND NOT EXISTS (
        SELECT 1 FROM public.student_location_attempts a
        WHERE a.user_id = p_user_id AND a.location_id = l.id AND a.medium = 'video_note' AND a.passed
      )
  );
$function$;
