-- Тренажёр местописаний БОЛЬШЕ НЕ обязателен для сдачи блока (остаётся как
-- учебный режим, не влияет на прохождение). «Закрытый день» = 4 задания за дату:
-- фото креста + молитва + местописания(audio) + пересказ(video_note).
