-- Фикс student_days: местописания (loc_done) считаются закрытыми только когда
-- сделаны ВСЕ обязательные стихи видеокружком (как в каноничном closed_dates_all),
-- а не «хотя бы одно». Плюс добавлен канонический столбец closed = день закрыт
-- (все 4 источника по одному блоку), чтобы отчёт совпадал с «ДНЕЙ ЗАКРЫТО».

DROP FUNCTION IF EXISTS public.student_days(uuid[]);

CREATE FUNCTION public.student_days(p_user_ids uuid[])
RETURNS TABLE(
...
AS $$
  WITH req AS (
    SELECT block_id, count(*) AS req FROM public.block_locations_to_recite WHERE is_required GROUP BY block_id
  ),
  loc_complete AS ( -- (user, block, d): все required местописания закрыты видеокружком
  ...
  closed_ud AS ( -- (user, d): канонически закрытый день (все 4 источника по одному блоку)
