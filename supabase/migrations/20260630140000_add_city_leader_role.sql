-- Роль «лидер города» (city_leader): иерархия super_admin → admin → city_leader →
-- curator → student. Лидер привязан к городу (profiles.city_id), один лидер на город.
-- (Решение Михаила 2026-06-30.)

ALTER TABLE public.profiles DROP CONSTRAINT IF EXISTS profiles_role_check;
ALTER TABLE public.profiles ADD CONSTRAINT profiles_role_check
  CHECK (role = ANY (ARRAY['student', 'curator', 'city_leader', 'admin', 'super_admin']));

-- is_visible_to: лидер города видит ВСЕХ в своём городе (кураторов + учеников — по
-- city_id ученика ИЛИ по городу его куратора). Куратор дополнительно видит своего
-- лидера города. admin/super_admin — всех (как было).
...
BEGIN
  ...
  IF viewer_role = 'city_leader' THEN
    ...
    IF EXISTS (SELECT 1 FROM profiles WHERE id = target_id AND city_id = viewer_city) THEN RETURN TRUE; END IF;
